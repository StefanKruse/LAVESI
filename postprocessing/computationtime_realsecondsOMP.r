...
barplot(matrix(c(as.vector(apply(t_N_poll[1:10,4:6],2,mean)/sum(apply(t_N_poll[1:10,4:6],2,mean))),as.vector(apply(t_N_poll[11:19,4:6],2,mean)/sum(apply(t_N_poll[11:19,4:6],2,mean))),as.vector(apply(t_N_poll[20:29,4:6],2,mean)/sum(apply(t_N_poll[20:29,4:6],2,mean)))),byrow=FALSE,ncol=3, nrow=3), names.arg=c("Base", "8 cores", "8 cores + pollination"), legend.text=c("polli","treemort","seedmort"))
