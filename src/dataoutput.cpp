...
#pragma omp parallel default(shared)
        {
            // declarations
            int localageg0 = 0, localageg1 = 0, localageg2 = 0, localageg3 = 0, localageg4 = 0, localageg5 = 0, localageg6b10 = 0, localageg11b20 = 0, localageg21b50 = 0, localageg51b100 = 0, localageg101b150 = 0, localageg151b200 = 0, localageg201b300 = 0, localageg301b400 = 0, localageg401b500 = 0, localageg501b600 = 0, localageg601b700 = 0, localageg701plus = 0;
            int localages0 = 0, localages1 = 0, localages2 = 0, localages3 = 0, localages4 = 0, localages5 = 0, localages6b10 = 0, localages11b20 = 0, localages21b50 = 0, localages51b100 = 0, localages101b150 = 0, localages151b200 = 0, localages201b300 = 0, localages301b400 = 0, localages401b500 = 0, localages501b600 = 0, localages601b700 = 0, localages701plus = 0;
...
// increase here number to +1 of included species
				...
				// aggregate data on grid with a set size e.g. 30 m or based on the input resolution as implemented here
...
#pragma omp parallel for default(shared) schedule(guided)
		...
		#pragma omp parallel for default(shared) schedule(guided)
					for (unsigned long long int tree_i = 0; tree_i < (unsigned long long int) tree_list.size(); ++tree_i) {
					   ...
					   if ((tree.species == speciesnumber_i) & (tree.growing == true)) {
									  // calculate grid position from x/y coordinates
									   unsigned int grid_i = floor((double)tree.ycoo / 1000 / parameter[0].demresolution*parameter[0].demresolution) * deminputdimension_x + floor((double)tree.xcoo / 1000 / parameter[0].demresolution*parameter[0].demresolution); //90 entered here (before: 30, changed because of DEM resolution)
					...
					for (unsigned int grid_i = 0; grid_i < AGBneedleliving.size(); ++grid_i) {
						...
						fprintf(filepointer, "%d;", x);
						fprintf(filepointer, "%d;", y);
						fprintf(filepointer, "%4.8f;", AGBneedleliving[grid_i] / 1000 / (parameter[0].demresolution*parameter[0].demresolution));    // in kg/sq.m.		//90 entered for all lines 1316-1332 (before: 30, changed because of DEM resolution)
						fprintf(filepointer, "%4.8f;", AGBwoodliving[grid_i] / 1000 / (parameter[0].demresolution*parameter[0].demresolution));      // in kg/sq.m.
						fprintf(filepointer, "%d;", (((100 * 100) / (parameter[0].demresolution*parameter[0].demresolution))) * Stemcount[grid_i]);  // in stems/ha
						fprintf(filepointer, "%4.8f;", ((100 * 100) / (parameter[0].demresolution*parameter[0].demresolution)) * Basalarea[grid_i]);
						fprintf(filepointer, "%d;", (((100 * 100) / (parameter[0].demresolution*parameter[0].demresolution))) * Indicount_10[grid_i]);          // in individuals/ha
