...
{ 
	...
	if (parameter[0].lineartransekt==true)
	{
		...
		{
			if(pTree->species==1)//wo wird über die tree_list iteriert?
			{
...
{
	/*
	signed int wachsabbruch;
	// Abfrage ob das Programm beendet oder fortgesetzt werden soll
	printf("\n Beginn des Wachstums. Weiter mit 1, beenden mit irgendeiner Eingabe\n");
	scanf("%d", &wachsabbruch);
	...
	*/
	...
	{ 
		...
		{ 
			...
			//Formeln beruhen auf reinen Annahmen, bzw. Ableitung von Beobachtungen. Hier wird das aktuelle Basalwachstum durch die Eigendimension nochmals verstärkt. Dadurch soll Ausgleichswachstum representiert werden. Es wirkt sich dann auch das relative Wachstum und darauf aufbauende Prozesse aus.
			...
			// Die height des aktuellen Treees aktualisieren
			if (pTree->height<130.0)
			{
			...
			{ // Sicherheitsabfrage height-Variable gesetzt?  
				...
				if (abbrechenwachstumfehler!=1) {printf("LaVeSi wurde nach einem Fehler in der Wachstumsfunktion beendet\n\n");
				exit(1);}
				delete pTree;
