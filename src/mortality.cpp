...
#pragma omp parallel for default(shared) private(uniform) schedule(guided)
    for (unsigned int tree_i = 0; tree_i < tree_list.size(); ++tree_i) {
        ...
        if (tree.growing == true) {
			...
			// treemortality = treemortality + (double)tree.firedamage * pow((100*1/(double)tree.height), 0.5*0.5); //adapted version from Stefan
			treemortality = treemortality + (double)tree.firedamage * pow((100*1/(double)tree.height), 0.5); //adapted version from Stefan,  toned down
