...
void Parameterinput(void) {
    ...
    parameter[0].weatherchoice = (long long int)Parameterinput(&uebergabestring[0], stringlengthmax, divisionsign, &parameter[0]);
    if (parameter[0].parameterinputvis == true) {
        printf("read:	%s <= %lld ==> Main\n", uebergabestring, parameter[0].weatherchoice);
    }
	...
	parameter[0].plotcentre_lat_start = parameter[0].plotcentre_lat;
	...
	parameter[0].plotcentre_lon_start = parameter[0].plotcentre_lon;
    strcpy(uebergabestring, "plotcentre_lat_end");
    parameter[0].plotcentre_lat_end = Parameterinput(&uebergabestring[0], stringlengthmax, divisionsign, &parameter[0]);
    if (parameter[0].parameterinputvis == true) {
        printf("read:	%s <= %4.3f ==> Main \n", uebergabestring, parameter[0].plotcentre_lat_end);
    }
	strcpy(uebergabestring, "plotcentre_lon_end");
    parameter[0].plotcentre_lon_end = Parameterinput(&uebergabestring[0], stringlengthmax, divisionsign, &parameter[0]);
    if (parameter[0].parameterinputvis == true) {
        printf("read:	%s <= %4.3f ==> Main \n", uebergabestring, parameter[0].plotcentre_lon_end);
    }
    ...
    strcpy(uebergabestring, "startatyear");
    parameter[0].startatyear = (int)Parameterinput(&uebergabestring[0], stringlengthmax, divisionsign, &parameter[0]);
    if (parameter[0].parameterinputvis == true) {
        printf("read:	%s <= %d ==> Main\n", uebergabestring, parameter[0].startatyear);
    }
...
void Getspeciestraits(void) {	
	cout << endl << " ... start reading species parameters ... " << endl;
	...
	cout << endl << " ... start reading species colonization times ... " << endl;
	...
	if (f2 == NULL) {
		printf("speciescolonizationtimes_....csv for the specified roi file not available!\n");
		exit(1);
	}
	...
	while( fgets(puffer2,1255,f2) !=NULL) {
		if (counter2>=1) { // skip header line
			...
			// speciescolonizationtimes[counter2].speciestimes.reserve(50);
			// speciescolonizationtimes[counter2].speciestimes.push_back(0);
			for(int i=1; i<=22; i++) {
				auto valuex = stoi(strtok(NULL, " "),NULL);
				// cout << valuex << "\t" << endl;
				// speciescolonizationtimes[counter2].speciestimes.push_back(valuex);
				speciescolonizationtimes[counter2].speciestimes[i] = valuex;
			}

			cout << speciescolonizationtimes[counter2].location << " :: " << speciescolonizationtimes[counter2].speciestimes[22] << endl;
		}
		counter2++;
	}
	fclose(f2);
	...
	// fill container for random seed introduction
	// ... based on weather_choice
	cout << "Species for random selection: ";
	long int plotcodeNum;
	plotcodeNum = parameter[0].weatherchoice % 1000;
	for (int species_counter = 1; species_counter <= 99; species_counter++) {
		if( (parameter[0].roi == (unsigned int)speciestrait[species_counter].roi) 
			&& ((unsigned int)speciescolonizationtimes[plotcodeNum].speciestimes[species_counter] < 999999) ) {
...
void ReadFire(void) {
	...
	for (unsigned int i = 0; i < parameter[0].n_weather_along_grid; ++i) {// weather grids in weather list
		...
		if(parameter[0].n_weather_along_grid > 0) {// get for i the position along start and end locations
			double i_step = (double) i / parameter[0].n_weather_along_grid;
			parameter[0].plotcentre_lat = parameter[0].plotcentre_lat_start + i_step * (parameter[0].plotcentre_lat_end - parameter[0].plotcentre_lat);
		...
		cout << "Reading global fire input ... ";
		while( fgets(buffer,1255,f_fire) != NULL ) {
			if (counter>=1) { // skip header line
				...
				if( (fabs(parameter[0].plotcentre_lat-lat_i) < parameter[0].firelocdistance) // 1 degree distance allowed and 0.5 degrees steps
					...
					distance = std::hypot(double(parameter[0].plotcentre_lat - lat_i), double(parameter[0].plotcentre_lon));
				} else {
					cout << ".";
				}
				...
				if(closelocation == true) {
					//store information
					globalfireparameter[counter_inradius].number[i] = number_i;
					globalfireparameter[counter_inradius].distance[i] = distance;
						cout << endl << "In radius, row: " << counter_inradius << " => " << globalfireparameter[counter_inradius].number[i] ;
					globalfireparameter[counter_inradius].latitude[i] = lat_i;
					globalfireparameter[counter_inradius].longitude[i] = lon_i;
						cout << " / Latitude: " << globalfireparameter[counter_inradius].latitude[i];
						cout << " & Longitude: " << globalfireparameter[counter_inradius].longitude[i] << endl;
					...
					globalfireparameter[counter_inradius].parameter1[i] = strtod(strtok(NULL, " " ),NULL);
					globalfireparameter[counter_inradius].parameter2[i] = strtod(strtok(NULL, " " ),NULL);
					globalfireparameter[counter_inradius].parameter3[i] = strtod(strtok(NULL, " " ),NULL);
					globalfireparameter[counter_inradius].parameter4[i] = strtod(strtok(NULL, " " ),NULL);
					globalfireparameter[counter_inradius].parameter5[i] = strtod(strtok(NULL, " " ),NULL);
					globalfireparameter[counter_inradius].parameter6[i] = strtod(strtok(NULL, " " ),NULL);
					globalfireparameter[counter_inradius].parameter7[i] = strtod(strtok(NULL, " " ),NULL);
					globalfireparameter[counter_inradius].parameter8[i] = strtod(strtok(NULL, " " ),NULL);
					globalfireparameter[counter_inradius].parameter9[i] = strtod(strtok(NULL, " " ),NULL);
					globalfireparameter[counter_inradius].threshold_mild[i] = strtod(strtok(NULL, " " ),NULL);
					// globalfireparameter[counter_inradius].threshold_medium = 0;
					// globalfireparameter[counter_inradius].threshold_severe = 0;
						cout << "threshold_mild: " << globalfireparameter[counter_inradius].threshold_mild[i] << endl;
				}
			}
			counter++;
		}
		...
		for(int counter_inradius_i = 1; counter_inradius_i <= counter_inradius; counter_inradius_i++) {
			sum_distance += globalfireparameter[counter_inradius_i].distance[i];
		}
		double sum_distance_perm = 0.0;
		for(int counter_inradius_i = 1; counter_inradius_i <= counter_inradius; counter_inradius_i++) {
			sum_distance_perm += sum_distance/globalfireparameter[counter_inradius_i].distance[i];
		}
		cout << " ... sum_distance = " << sum_distance << endl;
			globalfireparameter[0].parameter1[i] = 0.0;
			globalfireparameter[0].parameter2[i] = 0.0;
			globalfireparameter[0].parameter3[i] = 0.0;
			globalfireparameter[0].parameter4[i] = 0.0;
			globalfireparameter[0].parameter5[i] = 0.0;
			globalfireparameter[0].parameter6[i] = 0.0;
			globalfireparameter[0].parameter7[i] = 0.0;
			globalfireparameter[0].parameter8[i] = 0.0;
			globalfireparameter[0].parameter9[i] = 0.0;
			globalfireparameter[0].threshold_mild[i] = 0.0;
		for(int counter_inradius_i = 1; counter_inradius_i <= counter_inradius; counter_inradius_i++) {
			double weight_i = ((sum_distance/globalfireparameter[counter_inradius_i].distance[i])/sum_distance_perm);
			globalfireparameter[0].parameter1[i] += globalfireparameter[counter_inradius_i].parameter1[i] * weight_i;
			globalfireparameter[0].parameter2[i] += globalfireparameter[counter_inradius_i].parameter2[i] * weight_i;
			globalfireparameter[0].parameter3[i] += globalfireparameter[counter_inradius_i].parameter3[i] * weight_i;
			globalfireparameter[0].parameter4[i] += globalfireparameter[counter_inradius_i].parameter4[i] * weight_i;
			globalfireparameter[0].parameter5[i] += globalfireparameter[counter_inradius_i].parameter5[i] * weight_i;
			globalfireparameter[0].parameter6[i] += globalfireparameter[counter_inradius_i].parameter6[i] * weight_i;
			globalfireparameter[0].parameter7[i] += globalfireparameter[counter_inradius_i].parameter7[i] * weight_i;
			globalfireparameter[0].parameter8[i] += globalfireparameter[counter_inradius_i].parameter8[i] * weight_i;
			globalfireparameter[0].parameter9[i] += globalfireparameter[counter_inradius_i].parameter9[i] * weight_i;
			if(globalfireparameter[counter_inradius_i].threshold_mild[i] == 99999) {
				globalfireparameter[counter_inradius_i].threshold_mild[i] = 0.0;
			}
			globalfireparameter[0].threshold_mild[i] += globalfireparameter[counter_inradius_i].threshold_mild[i] * weight_i;
		}
	} // end loop weather grid	
