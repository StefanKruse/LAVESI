...
extern void Weatherinput(Parameter* parameter, int stringlengthmax, vector<vector<Weather>>& world_weather_list) {
    ...
    for (vector<vector<Weather>>::iterator posw = world_weather_list.begin(); posw != world_weather_list.end(); posw++) {
		...
		if ((parameter[0].windsource != 0) && (parameter[0].windsource != 4) && (parameter[0].windsource != 5)) {// start wind input
			...
			cout << "usingWinddata = " << foldername.str() << endl;
		...
		std::copy(windspd.begin(), windspd.end(), std::back_inserter(weather_list[0].windspd)); 
		std::copy(winddir.begin(), winddir.end(), std::back_inserter(weather_list[0].winddir)); 
		std::copy(globalyears.begin(), globalyears.end(), std::back_inserter(weather_list[0].globalyears)); 
		
		cout << " weather_list[0].windspd.size = " << weather_list[0].windspd.size();
		cout << " weather_list[0].globalyears.size = " << weather_list[0].globalyears.size() << endl;
		
		// clean global used variables
		windspd.clear();
		winddir.clear();
		globalyears.clear();
